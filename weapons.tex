...
\NewDocumentCommand{\gurps@wctemplate@preamble}{mm}{
  ...
  \def\xsep{%
    \discretionary{}{}{%
      \rule{1ex}{0pt}%
      \rule[-.4\baselineskip]{0.3pt}{1.2\baselineskip}%
      \rule{1ex}{0pt}%
  }}
  ...
  (points cost) {\scalebox{1.5}{%
      ...
      \begin{tabular}{@{}c@{}}%
        \IfInteger{\gurps@wc@points}{\tiny Points:\tabularnewline}{}
        % \\%
...
\define@key{MeleeWeaponCard}{points}{\edef\gurps@wc@points{#1}}
...
\define@key{RangedWeaponCard}{points}{\edef\gurps@wc@points{#1}}
\NewDocumentCommand{\RangedWeaponCard}{O{\small}m}{%
  ...
  \begin{tikzpicture}%
    ...
    \pgfmathsetmacro{\mpymax}{\ymax*0.90}
    \node [below = 0pt of picture top right, anchor=north west] (name) {%
      \begin{minipage}[t][\mpymax cm][s]{3.5cm}%
        \raggedright%
        ...
        \gurps@wc@attr{RoF}{\gurps@wc@rof}\xsep%
        \gurps@wc@attr{Range}{\gurps@wc@range}\xsep%
        \gurps@wc@attr{Bulk}{\gurps@wc@bulk}\xsep%
