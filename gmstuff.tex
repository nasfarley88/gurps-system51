...
\begin{tcolorbox}[title={Oh, that's wierd!}, parbox=false]
  ...
  A random NPC (\npc{John Kravitz}) comes up to \player{(Kevin)} and thinks he
  ...
  *** \player{(Kevin)} can choose to automatically remember, or roll IQ to remember. It's a game after all.
\end{tcolorbox}
...
\item Immediately afterwards, \npc{Kyoko} looks at \player{Xi Fau}, on a
  successful \rollcheck{Per}[-6] or \rollcheck{Body Language} roll they
  determine that she's not happy about something
\item \npc{Debbie} also rushes in, sees the hand, and apologises and blacks out
  the windows (like polarising glasses) and \player{Dorei} leaves
...
\item Followed by \npc{Debbie} coming out, neatening her dress, and smiling.
...
\item If it hasn't happened yet, let \npc{John Kravitz} show up in the office as
  a guest of \npc{Kyoko}, but leave when they start talking
\item \npc{Kyoko} takes the team into her office \emph{with some guards}, tells
  ...
  \emph{\npc{Kyoko} and the guards leave the room, making excuses about getting
    the paperwork}
  \begin{itemize}
  \item At first, don't do anything, just leave them in the room
  \item All roll \rollcheck{Per}[-6] (including the phox!) for the first five minutes. 
  \item Then roll \rollcheck{Per}[-3]
  \item Then roll \rollcheck{HT}[+2]
  \item Then roll \rollcheck{HT}[-2]
  \end{itemize}
...
\item The \monster{Catolin} appears in short order, destroying a guest and
...
\getcharacterfromgcx*{kyle}{Kyle}
\getcharacterfromgcx*{nasty_mcshandy}{Nasty McShandy}
\getcharacterfromgcx*{thomas}{Thomas}
\getcharacterfromgcx*{rufus_grumps_deal}{Grumps}
\getcharacterfromgcx*{xi_fau}{Xi Fau}

\foreach \i in {James Baron,Kyle,Nasty McShandy,Thomas,Grumps,Xi Fau} {
\luadirect{_GCHARACTERKEY = \luastring{\i}}
...
\section{NPC cards for the players}
\label{sec:npccardsforplayers}

\clearpage
\begin{tikzpicture}[remember picture,overlay,placer/.style={outer sep=0pt,inner sep=0pt,rotate=90}]
  \node[placer,anchor=north west] at (memoir page.center) {\noindent\SetCharacterKey{Thomas}\GCCardPrint\ResetCharacterKey};
  \node[placer,anchor=north east] at (memoir page.center) {\noindent\SetCharacterKey{Nasty McShandy}\GCCardPrint\ResetCharacterKey};
  \node[placer,anchor=south east] at (memoir page.center) {\noindent\SetCharacterKey{Kyle}\GCCardPrint\ResetCharacterKey};
  \node[placer,anchor=south west] at (memoir page.center) {\noindent\SetCharacterKey{Xi Fau}\GCCardPrint\ResetCharacterKey};
\end{tikzpicture}
\clearpage
\begin{tikzpicture}[remember picture,overlay,placer/.style={outer sep=0pt,inner sep=0pt,rotate=90}]
  \node[placer,anchor=north west] at (memoir page.center) {\noindent\SetCharacterKey{James Baron}\GCCardPrint\ResetCharacterKey};
  \node[placer,anchor=north east] at (memoir page.center) {\noindent\SetCharacterKey{Grumps}\GCCardPrint\ResetCharacterKey};
  \node[placer,anchor=south east] at (memoir page.center) {\noindent\SetCharacterKey{Jeff Bridges}\GCCardPrint\ResetCharacterKey};
\end{tikzpicture}
\clearpage
